...
# This is done at module level to avoid reloading models on each request
...
def get_reader() -> easyocr.Reader:
    ...
    global _reader
    if _reader is None:
        print("Initializing EasyOCR reader (ja, en)...")
...
def extract_structured_data(ocr_results: list) -> dict:
    """
    ...
    # Birth date patterns
    # Japanese format: 1990年01月15日 or 1990/01/15
    date_patterns = [
    ...
    # Expiry date (在留期間満了日)
    expiry_keywords = ['満了', '期限', 'EXPIRY', 'UNTIL']
...
def process_zairyu_card(image_bytes: bytes) -> dict:
    """
    Specifically process a Zairyu (residence) card.
    """
    result = process_image(image_bytes)

    # Add card-specific processing hints
    result['document_type'] = 'zairyu_card'

    return result


def process_rirekisho(image_bytes: bytes) -> dict:
    """
    Process a Rirekisho (resume) document.
    """
    result = process_image(image_bytes)

    # Add resume-specific processing
